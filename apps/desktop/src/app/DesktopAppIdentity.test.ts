...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Apna Chat.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Apna Chat.app/Contents/Resources",
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("Apna Chat (Alpha)")),
            readFileString: () =>
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Apna Chat (Alpha)");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Apna Chat (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Apna Chat (Alpha)");
