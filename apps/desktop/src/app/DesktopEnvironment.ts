...
const APP_BASE_NAME = chattuConfig.name;
...
const make = Effect.fn("desktop.environment.make")(function* (
  ...
  const baseDir = Option.getOrElse(config.t3Home, () =>
    path.join(homeDirectory, chattuConfig.desktop.homeDirectory),
  );
  ...
  const userDataDirName = isDevelopment
    ? `${chattuConfig.desktop.dataDirectory}-dev`
    : chattuConfig.desktop.dataDirectory;
  const legacyUserDataDirName = isDevelopment ? "Chattu (Dev)" : "Chattu (Alpha)";
  ...
  return DesktopEnvironment.of({
    ...
    appUserModelId: Option.getOrElse(config.appUserModelIdOverride, () =>
      isDevelopment ? `${chattuConfig.desktop.bundleId}.dev` : chattuConfig.desktop.bundleId,
    ),
    linuxDesktopEntryName: isDevelopment ? "chattu-dev.desktop" : "chattu.desktop",
    linuxWmClass: isDevelopment ? "chattu-dev" : "chattu",
    userDataDirName,
